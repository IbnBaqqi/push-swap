...
/*   Updated: 2025/01/04 02:37:19 by sabdulba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	mover(t_node **head_ref, int mid_val, int prev_val, int next_val)
{
	if (mid_val > prev_val && mid_val > next_val)
	{
		if (prev_val > next_val)
			...
			swap(*head_ref, (*head_ref)->next, "sa", 1);
	}
...
void	sort_three(t_node **head_ref, char is_cyclic)
{
	int				prev_val;
	t_node			*mid_node;
	int				next_val;
	...
	if (check_sort(head_ref) || (is_cyclic == 'y' && check_cyclic(head_ref)))
		return ;
	if (list_size(head_ref) == 2)
		return (swap(*head_ref, (*head_ref)->next, "sa", 1));
